...
class GovernanceClass(object):
    ...
    def vote(self, quantisnetd, signal, outcome):
        return self.go.vote(quantisnetd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, quantisnetd):
        if self.is_valid(quantisnetd):
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(quantisnetd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(quantisnetd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to quantisnetd "gobject list" output
    ...
    def quantisnetd_serialise(self):
        import quantisnetlib
        return quantisnetlib.SHIM_serialise_for_quantisnetd(self.serialise())
