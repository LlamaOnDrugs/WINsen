...
def test_valid_quantisnet_address():
    from quantisnetlib import is_valid_quantisnet_address

    main = valid_quantisnet_address()
    test = valid_quantisnet_address('testnet')

    assert is_valid_quantisnet_address(main) is True
    assert is_valid_quantisnet_address(main, 'mainnet') is True
    assert is_valid_quantisnet_address(main, 'testnet') is False

    assert is_valid_quantisnet_address(test) is False
    assert is_valid_quantisnet_address(test, 'mainnet') is False
...
def test_invalid_quantisnet_address():
    ...
    test = invalid_quantisnet_address('testnet')

    assert is_valid_quantisnet_address(main) is False
    assert is_valid_quantisnet_address(main, 'mainnet') is False
    assert is_valid_quantisnet_address(main, 'testnet') is False

    assert is_valid_quantisnet_address(test) is False
    assert is_valid_quantisnet_address(test, 'mainnet') is False
    assert is_valid_quantisnet_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from quantisnetlib import elect_mn
...
def test_hash_function():
    ...
    hex_hash = "%x" % quantisnetlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert int(quantisnetlib.blocks_to_seconds(16616)) == 2113555
